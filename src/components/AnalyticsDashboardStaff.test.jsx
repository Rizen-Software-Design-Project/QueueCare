...
const defaultWaitData = [
    { hour_of_day: 9,  avg_wait_minutes: 12, min_wait_minutes: 5, max_wait_minutes: 25, total_served: 30 },
    { hour_of_day: 14, avg_wait_minutes: 20, min_wait_minutes: 8, max_wait_minutes: 40, total_served: 50 },
];

const defaultNoShowData = [
    { date: "2024-06-02", no_show_rate_pct: "28.0", no_shows: "7", total_appointments: "25" },
    { date: "2024-06-01", no_show_rate_pct: "30.0", no_shows: "3", total_appointments: "10" },
];

const lowNoShowData = [
    { date: "2024-06-02", no_show_rate_pct: "5.0",  no_shows: "1", total_appointments: "20" },
    { date: "2024-06-01", no_show_rate_pct: "6.7",  no_shows: "2", total_appointments: "30" },
];
...
const defaultCustomData = [
    {
        ...
        patient_name: "Jane", patient_surname: "Dlamini",
        patient_email: "jane@example.com", patient_phone: null,
        appointment_type: "scheduled", status: "complete",
        queue_status: "completed", wait_minutes: 10, service_minutes: 15,
    },
    {
        id: "appt-cv-2",
        booked_at: "2024-06-02T09:00:00Z",
        patient_name: "John", patient_surname: "Mokoena",
        patient_email: null, patient_phone: "0831234567",
        appointment_type: "walk_in", status: "no_show",
        queue_status: null, wait_minutes: null, service_minutes: null,
    },
];
...
function openAnalyticsHeader() {
    ...
    it("Wait Times tab is active by default", () => {
        expect(screen.getByRole("tab", { name: /wait times/i })).toHaveClass("tab-btn--active");
    });
}
...
function openCustomViewTableHeaders() {
    ...
    it('does NOT render a "Facility" column header', () => {
        const panel = getVisiblePanel();
        const facilityTh = [...panel.querySelectorAll("th")].find((t) => /^facility$/i.test(t.textContent.trim()));
        expect(facilityTh).toBeUndefined();
    });
}
...
describe("Analytics Staff - facility resolution - profile error", () => {
    it("shows a profile error message when the profiles query fails", async () => {
        localStorage.setItem("userIdentity", JSON.stringify({ auth_provider: "google", provider_user_id: "uid-1" }));
        const chain = { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), maybeSingle: vi.fn().mockResolvedValue({ data: null, error: { message: "db error" } }) };
        mockSupabaseFrom.mockReturnValue(chain);
        render(<AnalyticsDashboardStaff />);
...
describe("Analytics Staff - facility resolution - success", () => {
    ...
    it("passes the resolved facilityId to useWaitTimes", async () => {
        await renderStaff();
        expect(mockUseWaitTimes).toHaveBeenCalledWith(expect.objectContaining({ facilityId: 42 }));
    });
});
...
describe("Wait Times tab - single row data", () => {
    const singleRow = [{ hour_of_day: 9, avg_wait_minutes: 15, min_wait_minutes: 5, max_wait_minutes: 30, total_served: 10 }];
...
describe("Wait Times tab - hour label formatting", () => {
    it("formats hour 0 as '12 am'", async () => {
        mockUseWaitTimes.mockReturnValue({ data: [{ hour_of_day: 0, avg_wait_minutes: 5, min_wait_minutes: 2, max_wait_minutes: 10, total_served: 5 }], loading: false });
        await navigateToWaitTimesTab();
        expect(screen.getByText("12 am")).toBeVisible();
    });

    it("formats hour 12 as '12 pm'", async () => {
        mockUseWaitTimes.mockReturnValue({ data: [{ hour_of_day: 12, avg_wait_minutes: 18, min_wait_minutes: 5, max_wait_minutes: 35, total_served: 40 }], loading: false });
        await navigateToWaitTimesTab();
        expect(screen.getByText("12 pm")).toBeVisible();
    });

    it("formats afternoon hour 15 as '3 pm'", async () => {
        mockUseWaitTimes.mockReturnValue({ data: [{ hour_of_day: 15, avg_wait_minutes: 22, min_wait_minutes: 8, max_wait_minutes: 45, total_served: 60 }], loading: false });
        await navigateToWaitTimesTab();
...
describe("Wait Times tab - export buttons", () => {
    ...
    it("clicking PDF calls exportPDF with correct title and filename", async () => {
        ...
        expect(mockExportPDF).toHaveBeenCalledWith(
            expect.any(Array),
            expect.objectContaining({ title: expect.stringContaining("Wait Times"), filename: "wait-times" })
        );
    });
...
describe("Wait Times tab - tab switching", () => {
    it("clicking No-Show Rates tab hides the Wait Times panel", async () => {
        const user = await navigateToWaitTimesTab();
        await user.click(screen.getByRole("tab", { name: /no-show rates/i }));
        expect(document.getElementById("tabpanel-0")).toHaveAttribute("hidden");
    });
    ...
    it("clicking back to Wait Times tab shows its panel again", async () => {
        ...
        expect(document.getElementById("tabpanel-0")).not.toHaveAttribute("hidden");
    });

    it("No-Show Rates tab gets active class when clicked", async () => {
        const user = await navigateToWaitTimesTab();
        await user.click(screen.getByRole("tab", { name: /no-show rates/i }));
        expect(screen.getByRole("tab", { name: /no-show rates/i })).toHaveClass("tab-btn--active");
        expect(screen.getByRole("tab", { name: /wait times/i    })).not.toHaveClass("tab-btn--active");
    });
});
...
describe("No-Show Rates tab - computed stat values", () => {
    ...
    it("shows correct overall no-show rate", () => { expect(screen.getByText("28.6%")).toBeVisible(); });
    it("shows correct total no-shows",       () => { expect(screen.getByText("10")).toBeVisible(); });
    it("shows correct total appointments",   () => { expect(screen.getByText("35")).toBeVisible(); });
    it("shows correct days analysed",        () => { expect(screen.getByText("2")).toBeVisible(); });
});
...
describe("No-Show Rates tab - empty data", () => {
    ...
    it("shows — for overall no-show rate", () => { expect(screen.getByText("—%")).toBeVisible(); });
...
describe("No-Show Rates tab - date range filter", () => {
    it("setting a start date passes it to useNoShowRates", async () => {
        ...
        expect(mockUseNoShowRates).toHaveBeenCalledWith(expect.objectContaining({ startDate: "2024-06-01" }));
    });
    ...
    it("setting an end date passes it to useNoShowRates", async () => {
        ...
        expect(mockUseNoShowRates).toHaveBeenCalledWith(expect.objectContaining({ endDate: "2024-06-30" }));
    });
    ...
    it("clearing the start date passes null to useNoShowRates", async () => {
        ...
        expect(mockUseNoShowRates).toHaveBeenLastCalledWith(expect.objectContaining({ startDate: null }));
    });

    it("useNoShowRates is initially called with null startDate and null endDate", async () => {
        mockUseNoShowRates.mockReturnValue({ data: defaultNoShowData, loading: false });
        await navigateToNoShowTab();
        expect(mockUseNoShowRates).toHaveBeenCalledWith(expect.objectContaining({ startDate: null, endDate: null }));
    });

    it("useNoShowRates is called with the resolved facilityId, not null", async () => {
        mockUseNoShowRates.mockReturnValue({ data: defaultNoShowData, loading: false });
        await navigateToNoShowTab();
        expect(mockUseNoShowRates).toHaveBeenCalledWith(expect.objectContaining({ facilityId: 42 }));
    });
});
...
describe("No-Show Rates tab - export buttons", () => {
    ...
    it("clicking CSV calls exportCSV with chart data and filename 'noshows'", async () => {
        ...
        await user.click(csvBtn);
        expect(mockExportCSV).toHaveBeenCalledWith(
            expect.arrayContaining([expect.objectContaining({ "No-Show Rate %": 28 })]),
            "noshows"
        );
    });
    ...
    it("clicking PDF calls exportPDF with correct title and filename", async () => {
        ...
        expect(mockExportPDF).toHaveBeenCalledWith(
            expect.any(Array),
            expect.objectContaining({ title: expect.stringContaining("No-Show"), filename: "noshows" })
        );
    });
...
describe("Custom View tab - table data", () => {
    ...
    it("renders wait minutes with 'min' suffix", () => { expect(screen.getByText("10 min")).toBeVisible(); });
    it("renders service minutes with 'min' suffix", () => { expect(screen.getByText("15 min")).toBeVisible(); });
    ...
    it("shows correct records count",          () => { expect(screen.getByText("2 records")).toBeVisible(); });
    it("renders status badge for 'complete'",   () => { expect(screen.getByText("complete")).toBeVisible(); });
    it("renders status badge for 'no show'",    () => { expect(screen.getByText("no show")).toBeVisible(); });
});
...
describe("Custom View tab - status filter", () => {
    it("All Statuses is the default option", async () => {
        ...
        await navigateToCustomViewTab();
        const panel = getVisiblePanel();
        expect(panel.querySelectorAll("select")[0]).toHaveValue("");
    });
    ...
    it("selecting a status passes it to useCustomView", async () => {
        ...
        const panel = getVisiblePanel();
        await user.selectOptions(panel.querySelectorAll("select")[0], "no_show");
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ status: "no_show" }));
    });
    ...
    it("resetting status to All passes null to useCustomView", async () => {
        ...
        const user = await navigateToCustomViewTab();
        const panel = getVisiblePanel();
        await user.selectOptions(panel.querySelectorAll("select")[0], "booked");
        await user.selectOptions(panel.querySelectorAll("select")[0], "");
        expect(mockUseCustomView).toHaveBeenLastCalledWith(expect.objectContaining({ status: null }));
    });
});
...
describe("Custom View tab - type filter", () => {
    it("All Types is the default option", async () => {
        ...
        await navigateToCustomViewTab();
        const panel = getVisiblePanel();
        expect(panel.querySelectorAll("select")[1]).toHaveValue("");
    });
    ...
    it("selecting Scheduled passes 'scheduled' to useCustomView", async () => {
        ...
        const user = await navigateToCustomViewTab();
        const panel = getVisiblePanel();
        await user.selectOptions(panel.querySelectorAll("select")[1], "scheduled");
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ type: "scheduled" }));
    });
    ...
    it("selecting Walk-in passes 'walk_in' to useCustomView", async () => {
        ...
        const user = await navigateToCustomViewTab();
        const panel = getVisiblePanel();
        await user.selectOptions(panel.querySelectorAll("select")[1], "walk_in");
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ type: "walk_in" }));
    });
    ...
    it("resetting type to All passes null to useCustomView", async () => {
        ...
        const panel = getVisiblePanel();
        await user.selectOptions(panel.querySelectorAll("select")[1], "walk_in");
        await user.selectOptions(panel.querySelectorAll("select")[1], "");
        expect(mockUseCustomView).toHaveBeenLastCalledWith(expect.objectContaining({ type: null }));
    });
});
...
describe("Custom View tab - date range filter", () => {
    it("useCustomView is initially called with null startDate and null endDate", async () => {
        mockUseCustomView.mockReturnValue({ data: defaultCustomData, loading: false });
        await navigateToCustomViewTab();
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ startDate: null, endDate: null }));
    });
    ...
    it("setting a start date passes it to useCustomView", async () => {
        ...
        const [startInput] = panel.querySelectorAll("input[type='date']");
        fireEvent.change(startInput, { target: { value: "2024-06-01" } });
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ startDate: "2024-06-01" }));
    });
    ...
    it("setting an end date passes it to useCustomView", async () => {
        ...
        const [, endInput] = panel.querySelectorAll("input[type='date']");
        fireEvent.change(endInput, { target: { value: "2024-06-30" } });
        expect(mockUseCustomView).toHaveBeenCalledWith(expect.objectContaining({ endDate: "2024-06-30" }));
    });
    ...
    it("clearing the start date passes null to useCustomView", async () => {
        ...
        fireEvent.change(startInput, { target: { value: "2024-06-01" } });
        fireEvent.change(startInput, { target: { value: "" } });
        expect(mockUseCustomView).toHaveBeenLastCalledWith(expect.objectContaining({ startDate: null }));
    });
});
...
describe("Custom View tab - export buttons", () => {
    ...
    it("clicking PDF calls exportPDF with correct title and filename", async () => {
        ...
        expect(mockExportPDF).toHaveBeenCalledWith(
            expect.any(Array),
            expect.objectContaining({ title: expect.stringContaining("Custom"), filename: "appointments" })
        );
    });
...
describe("Custom View tab - initial hook call args", () => {
    it("useCustomView is called with all null filters and the resolved facilityId on first render", async () => {
        ...
        expect(mockUseCustomView).toHaveBeenCalledWith(
            expect.objectContaining({ facilityId: 42, status: null, type: null, startDate: null, endDate: null })
        );
    });
});
...
describe("Custom View tab - tab switching", () => {
    it("Custom View tab gets the active class when clicked", async () => {
        mockUseCustomView.mockReturnValue({ data: defaultCustomData, loading: false });
        await navigateToCustomViewTab();
        expect(screen.getByRole("tab", { name: /custom view/i })).toHaveClass("tab-btn--active");
        expect(screen.getByRole("tab", { name: /wait times/i  })).not.toHaveClass("tab-btn--active");
    });
