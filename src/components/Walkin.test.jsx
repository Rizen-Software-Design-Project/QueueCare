...
// The slots query in fetchSlots is:
//   supabase.from("appointment_slots").select(...).eq(...).order(...).order(...)
// Two chained .order() calls. The second .order() must resolve with the slots data.
// We model this with a dedicated slotsOrder mock that always returns this,
// and a slotsOrderFinal mock that resolves with data — applied to the second .order().
