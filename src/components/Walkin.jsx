...
export default function WalkIn({
  ...
  async function fetchSlots(fId) {
    const { data } = await supabase
      ...
      .select(
        "id, slot_date, slot_time, duration_minutes, total_capacity, booked_count"
      )
      .eq("facility_id", fId)
  ...
  async function handleSearch(e) {
    ...
    setSearchMsg({
      type: "success",
      text: "Profile found — confirm below.",
    });
  }
  ...
  async function handleSubmit(e) {
    ...
    const bookRes = await fetch(
      `${API_BASE}/appointments/book-walkin`,
      {
        method: "POST",
        headers: {
    ...
    if (activeTab === "queue") {
      ...
      if (!qRes.ok || qData.error) {
        setSubmitMsg({
          ...
          text:
            qData.error ||
            "Appointment created but failed to add to queue.",
        });
  ...
  async function handleCheckIn(e) {
    ...
    const { data: appts } = await supabase
      .from("appointments")
      .select(
        "id, appointment_slots!inner(slot_date, facility_id)"
      )
      .eq("patient_id", profile.id)
      .eq(
        "appointment_slots.facility_id",
        facilityId
      )
      .eq(
    ...
    if (!qRes.ok || qData.error) {
      setSubmitMsg({
        type: "error",
        text:
          qData.error || "Failed to add to queue.",
      });
  ...
  const todaySlots = slots.filter(
    (s) =>
      ...
      (s.total_capacity ?? 0) >
        (s.booked_count ?? 0) &&
      new Date(`${s.slot_date}T${s.slot_time}`) >
        now
  );
            ...
            <>
              <article className="wi-profile-card">
                <header className="wi-profile-card-top">
                  ...
                  <strong className="wi-profile-name">
                    {profile.name} {profile.surname}
                  </strong>
                </header>

                <ul className="wi-profile-details">
                  <li className="wi-profile-detail">
                    📧 {profile.email || "—"}
                  </li>
