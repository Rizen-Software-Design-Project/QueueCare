...
export default function AdminClinics() {
  ...
  const availableDistricts = useMemo(() => {
    return province && districtsByProvince[province]
  ...
  async function applyFilters() {
    ...
    try {
      ...
      setStatus({
        ...
        message: data.length
          ? `Showing ${data.length} facilities`
          : "No facilities found",
      });
    } catch (err) {
 ...
 async function saveFacilityChanges() {
    ...
    prev.map((f) =>
      f.id === editingFacility.id
        ? { ...f,
            ...
            services_offered: editingFacility.services_offered,
            operating_hours:  cleanedHours }
        : f
    )
  );
  ...
  return (
    <div className="admin-container">
      <h2 className="title">Clinic Management</h2>
        ...
        {filteredFacilities.map((facility) => (
          <div key={facility.id} className="card">
            <div>
              ...
              <p className={facility.is_active ? "active" : "inactive"}>
  ...
  <div className="services-tags">
    {SERVICE_OPTIONS.map((service) => {
      const selected =
        editingFacility.services_offered?.includes(service) ?? false;
            ...
            <div key={day} className="day-row">
              ...
              <label>
                <input
                  type="checkbox"
                  checked={
                    editingFacility.operating_hours_form?.[day]?.closed ?? false
                  }
                  onChange={(e) =>
