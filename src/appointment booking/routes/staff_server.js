...
router.get('/is_booked', async (req, res) => {
  ...
  try {
    ...
    if (error) throw error;
    if (data.length == 0) return res.status(404).json({ error: "Person does not have an account." });
...
router.get('/queue_position', async (req, res) => {
  ...
  const filtered = data.filter(e => e.virtual_queue !== null && e.virtual_queue?.status == "Waiting");
  filtered.sort((a, b) => {
    const dateTimeA = `${a.appointment_slots.slot_date}T${a.appointment_slots.slot_time}`;
    const dateTimeB = `${b.appointment_slots.slot_date}T${b.appointment_slots.slot_time}`;
    return new Date(dateTimeA) - new Date(dateTimeB);
  ...
  try {
    ...
    for (let entry of filtered) {
      position += 1;
      entry.position = position;
      if (entry.profiles.id == patient_id)
        return res.status(200).json({ position: entry.position });
    }
