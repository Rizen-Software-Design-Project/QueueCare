...
//checks if phonenumber exists

app.get('/isnewuser', async (req, res) => {  // ✅ missing comma after route
    const identity = req.query.identity;
    try {
        let data;  // ✅ declared outside so both blocks can access it
        ...
        if (data.length == 0) {  // ✅ check data.length not identity.length
            return res.status(200).json({ exists: false });
        } else {
            return res.status(200).json({ exists: true });
        }
